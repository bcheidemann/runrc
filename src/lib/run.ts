...
export async function run(alias: string, args: string[], diagnostics: DiagnosticsReporter): Promise<Result<void, Diagnostic>> {
  ...
  const command = config.options.commands.find((command) => command.alias === alias);

  if (!command) {
    const error = Diagnostic.error(`Command ${chalk.italic(alias)} not found`);
    diagnostics.addAndEmit(error);
    const info = Diagnostic.info(`Available commands: ${config.options.commands.map((command) => command.alias).join(", ")}`);
    diagnostics.addAndEmit(info);
    return Err(error);
  }

  const runner = command.runner ?? config.options.runner ?? { command: "bash", args: ["-c"] };
